...
export function SongCard({ song }: { song: Song }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-soft dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className={user ? "mt-auto grid grid-cols-2 gap-2 pt-4" : "mt-auto pt-4"}>
        {user ? (
          <Link
            to={`/chants/${song.id}`}
            className="inline-flex w-full items-center justify-center rounded-xl bg-brand-600 px-3 py-2 text-sm font-medium text-white hover:bg-brand-700"
          >
            Voir le détail
          </Link>
        ) : (
          <Button
            className="w-full"
            variant="secondary"
            onClick={() => navigate("/login", { state: { from: { pathname: "/chants" } } })}
          >
            Se connecter pour voir le détail
          </Button>
        )}
        <FavoriteButton songId={song.id} className="w-full" />
      </div>
    </article>
  );
}
